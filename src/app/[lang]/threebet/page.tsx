...
import { generateThreeBetScenario } from "@/lib/poker";
import type { ThreeBetScenario } from "@/lib/poker";
...
export default function ThreeBet() {
  const { dict } = useI18n();
  const t = (dict.threebet ?? {}) as Record<string, string>;
  const { requireAuth } = useAuthWall();

  const [scenario, setScenario] = useState<ThreeBetScenario | null>(null);
  ...
  const newScenario = useCallback(() => {
    ...
    setAnswer(null);
    setTimeout(() => {
      setScenario(generateThreeBetScenario(level));
      setComputing(false);
    }, 10);
  }, [level]);
  ...
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-16">
      <h1 className="text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
        {t.title ?? "3-Bet Situations"}
      </h1>
      <p className="mt-4 text-lg leading-8 text-zinc-600 dark:text-zinc-400">
        {t.intro ?? "You opened with a raise and your opponent 3-bets. Should you call or fold?"}
      </p>
        ...
        <div className="mt-8">
          ...
          <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: 360 }}>
            ...
            <div className="absolute left-1/2 top-[12%] -translate-x-1/2 z-10 flex flex-col items-center gap-2">
              ...
              <div className="rounded bg-red-600/80 border border-red-500 px-3 py-1.5 text-white font-bold text-sm">
                3-bet: {scenario.opponentThreeBet}
              </div>
            </div>

            {/* Pot + action info */}
            <div className="absolute left-1/2 top-[46%] -translate-x-1/2 z-10 flex flex-col items-center gap-1">
              ...
              <div className="mt-1 flex items-center gap-3 text-xs text-zinc-300">
                <span>{t.yourOpen ?? "Your open"}: {scenario.playerRaise}</span>
                <span className="text-zinc-500">|</span>
                <span>{t.toCallLabel ?? "To call"}: {scenario.toCall}</span>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 p-5 bg-white dark:bg-zinc-900">
                ...
                <div className="mt-3 grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-xs text-zinc-500 uppercase">{t.yourEquity ?? "Your equity vs 3-bet range"}</span>
                ...
                <p className="mt-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                  {(t.explanation ?? "You need {potOdds}% equity to call profitably. Against a typical 3-bet range you have {equity}% equity.")
              ...
              <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 p-5 bg-white dark:bg-zinc-900">
                ...
                <p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                  {(() => {
                    ...
                    if (diff > 20) return t.practStrong ?? "Your hand is very strong against the 3-bet range. Calling is clear, and you might even consider 4-betting for value.";
                    if (diff > 5) return t.practGood ?? "Solid call. Your hand has enough equity to profitably continue. In practice, suited connectors and strong broadways play well here.";
                    if (diff > -5) return t.practMarginal ?? "Marginal spot. The math is close. Consider: how tight is the opponent's 3-bet range? Do you have position? Hands with good playability (suited, connected) are better calls than raw high-card hands.";
                    if (diff > -15) return t.practBad ?? "Leaning toward a fold, but context matters. If your opponent 3-bets wide (bluffs often), calling can still work. Without reads, folding is safer.";
                    return t.practTerrible ?? "Clear fold. Your hand doesn't have the equity or playability to continue against a 3-bet. Save your chips.";
                  })()}
                </p>
              </div>
